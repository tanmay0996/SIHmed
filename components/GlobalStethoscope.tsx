...
import { Stethoscope } from "./MedicalIcons";
...
export default function GlobalStethoscope({
  debug = false,
  showFallback = false,
}: {
  debug?: boolean;
  ...
  // debug console
  useEffect(() => {
    if (debug) console.log("[GlobalStethoscope] controlPoints:", controlPoints);
  }, [debug]);
  ...
  return (
    <>
      {debug && (
        ...
        >
          <div style={{ fontWeight: 700, marginBottom: 6 }}>Stethoscope debug</div>
      ...
      <div aria-hidden style={containerStyle} className="pointer-events-none">
        ...
        >
          ...
          >
            ...
            >
              <Stethoscope className="w-72 h-72 md:w-96 md:h-96" />
            </div>
          </motion.div>
